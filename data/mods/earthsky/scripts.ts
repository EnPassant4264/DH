...
export const Scripts: ModdedBattleScriptsData = {
	...
	battle: {
		...
		getDamage( 
			...
			const moveCategory = this.getCategory(move);
			const defensiveCategory = move.defensiveCategory || moveCategory;
			...
			let attackStat: StatNameExceptHP = moveCategory === 'Physical' ? 'atk' : 'spa';
			...
			attackStat = (moveCategory === 'Physical' ? 'atk' : 'spa');
		...
		modifyDamage( //Tactician and Dual-type STAB
		...
		sourceEffect?: Effect | null, relayVar?: any, onEffect?: boolean, fastExit?: boolean
			...
			if (this.eventDepth >= 8) {
				// oh fuck <-- shout-out to this comment
			...
			for (const handler of handlers) {
				...
				if ((effect.effectType === 'Weather' || eventid === 'Weather') &&
					...
					this.debug(eventid + ' handler suppressed by Air Lock, Cloud Nine, or Midnight');
					continue;
				}
		...
		singleEvent( //Same here :((
			...
			if (effect.effectType === 'Terrain' && eventid !== 'Start' && eventid !== 'Residual' &&
				eventid !== 'End' && this.field.suppressingTerrain()) {
				this.debug(eventid + ' handler suppressed by Midnight');
				return relayVar;
			}
	...
	/*dex: {
		...
		getImmunity( //Dual-type move type calculation
			...
			if(typeof source !== 'string'){
				if(source.twoType){
					return !(this.getImmunity(source.type) && this.getImmunity(source.twoType));
				} else {
		...
		getEffectiveness( //Dual-type move type calculation
			source: {type: string} | string,
			target: {getTypes: () => string[]} | {types: string[]} | string[] | string
		): number {
			let sourceType: string = "";
			if(typeof source !== 'string'){
				if(source.twoType){
					return this.getEffectiveness(source.type) + this.getEffectiveness(source.twoType);
				} else {
					sourceType = source.type;
				}
			} else sourceType = source;
	...
	init() {
		...
		for (let pokemonID in this.data.Pokedex) {
			...
			if(pokemon.isNonstandard && pokemon.isNonstandard === "Past") { //Mark all Pokemon as current-gen
		...
		for(let itemID in this.data.Items){
			const item = this.modData('Items', itemID);
			if(item.megaStone) delete item.isNonstandard; //Release the Megas!
		...
		// Beautifly
		this.modData("Learnsets", "beautifly").learnset.dazzlinggleam = ["8D"];
		...
		this.modData("Learnsets", "rotomwash").learnset.hydropump = ["8R"];
		this.modData("Learnsets", "rotomwash").learnset.muddywater = ["8L57"];
		...
		delete this.modData('Learnsets', 'rotomwash').learnset.hex;
		...
		this.modData("Learnsets", "rotomheat").learnset.heatwave = ["8L57"];
		this.modData("Learnsets", "rotomheat").learnset.incinerate = ["8L27"];
		this.modData("Learnsets", "rotomheat").learnset.overheat = ["8R"];
		...
		delete this.modData('Learnsets', 'rotomheat').learnset.hex;
		...
		this.modData("Learnsets", "rotomfrost").learnset.blizzard = ["8R"];
		this.modData("Learnsets", "rotomfrost").learnset.flash = ["8M"];
		this.modData("Learnsets", "rotomfrost").learnset.freezedry = ["8L27"];
		this.modData("Learnsets", "rotomfrost").learnset.hail = ["8L1"];
		this.modData("Learnsets", "rotomfrost").learnset.icebeam = ["8L57"];
		...
		delete this.modData('Learnsets', 'rotomfrost').learnset.hex;
		...
		this.modData("Learnsets", "rotommow").learnset.energyball = ["8L57"];
		this.modData("Learnsets", "rotommow").learnset.flash = ["8M"];
		this.modData("Learnsets", "rotommow").learnset.grassyterrain = ["8L1"];
